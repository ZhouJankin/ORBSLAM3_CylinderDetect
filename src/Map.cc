...
//MapCylinder functions
vector<MapCylinder*> Map::GetAllCylinders() {
    unique_lock<mutex> lock(mMutexMap);
    return vector<MapCylinder*>(mspMapCylinders.begin(),mspMapCylinders.end());
}
...
MapCylinder* Map::GetCurCylinder() {
    unique_lock<mutex> lock(mMutexMap);
    return mpCurCylinder;
}

void Map::AddMapCylinder(MapCylinder* pCy) {
    unique_lock<mutex> lock(mMutexMap);
    mspMapCylinders.insert(pCy);
}

//地图中去除，地图点去除，跟该圆柱有关系的关键帧也要去除
void Map::EraseMapCylinder(MapCylinder* pCy) {
    unique_lock<mutex> lock(mMutexMap);
    pCy->SetBadFlag();
    mspMapCylinders.erase(pCy);
    mpCurCylinder = nullptr;
}
//
