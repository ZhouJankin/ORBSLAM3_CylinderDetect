...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    //     Eigen::Vector3d LandMark = pos.cast<double>();
    //     mvLandMarks.emplace_back(LandMark);
    // }
...
void Optimizer::OptimizeOnce(std::vector<Eigen::Vector3d> &noisyLandmarks, std::vector<bool> OuterIndex, CylinderIntrinsics &abc) {
    // Setup optimizer
    g2o::SparseOptimizer optimizer;
...
std::vector<double> Optimizer::computeDistance(const std::vector<Eigen::Vector3d> &noisyLandmarks, const CylinderIntrinsics &abc) {
    ...
    for(int i = 0; i < size; ++i) {
        ...
        double checkout = sqrt(u.transpose() * u);
        dists.push_back(sqrt(u.transpose() * u) - abc.r);
    }
    return dists;
}
...
double Optimizer::computeStdDev(const std::vector<double> &dists, double mean, const std::vector<bool> &isOuter, int InnerNum) {
    double accum = 0.;
    for(int i = 0; i < dists.size(); ++i) {
        // if(isOuter[i]) {
        //     continue;
...
bool Optimizer::LoopOptimization(std::list<MapPoint*> &lpPastMapPoints, MapCylinder* pCy) {
    ...
    do {
        ...
        std::vector<double> dists = computeDistance(noisyLandmarks, cur_abc);
        // double mean = computeMean(dists, isOuter, last_inner_size);
        // double stdDev = computeStdDev(dists, mean, isOuter, last_inner_size);
        double mean = computeMean(dists, isOuter, size);
        double stdDev = computeStdDev(dists, mean, isOuter, size);
        ...
        std::cout<<"第 "<<iteration<<"轮迭代,所有点数目为："<<size<<std::endl;
